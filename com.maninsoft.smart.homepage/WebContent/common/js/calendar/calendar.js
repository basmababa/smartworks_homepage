...
function Calendar(obj, ev) {													// jucke
	...
	minical.style.top	= y + document.body.scrollTop + 'px';
...
function doOver() {																// 마우스가 칼렌다위에 있으면
	var el = (mEvent.srcElement)? mEvent.srcElement : mEvent.target;
	cal_Day = el.title;
...
function doOut() {
	var el = (mEvent.fromElement)? mEvent.fromElement : mEvent.relatedTarget;
	cal_Day = el.title;
	if (cal_Day.length > 7) {
...
function doClick(cId) {															// 날자를 선택하였을 경우
	var el = (mEvent.srcElement)? mEvent.srcElement : document.getElementById(cId);
	if (el) {
		cal_Day = el.title;
	...
	for (intLoopWeek=1; intLoopWeek < 7; intLoopWeek++) {						// 주단위 루프 시작, 최대 6주
		...
		for (intLoopDay=1; intLoopDay <= 7; intLoopDay++) {						// 요일단위 루프 시작, 일요일 부터
			if (intThirdWeekday > 0) {											// 첫주 시작일이 1보다 크면
				Cal_HTML += "<TD onClick=doClick('" + thirdPrintDay + "');>";
				intThirdWeekday--;
			} else {
				if (thirdPrintDay > intLastDay) {								// 입력 날짝 월말보다 크다면
					Cal_HTML += "<TD onClick=doClick('" + thirdPrintDay + "');>";
				} else {														// 입력날짜가 현재월에 해당 되면
					Cal_HTML += "<TD id='" + thirdPrintDay + "' onClick=doClick('" + thirdPrintDay + "'); title="+intThisYear+"-"+day2(intThisMonth).toString()+"-"+day2(thirdPrintDay).toString()+" STYLE=\"cursor:pointer;border:1px solid white;";
