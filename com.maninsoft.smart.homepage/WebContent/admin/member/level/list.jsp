...
<%
	...
	ArrayList levelArray = LevelManager.getInstance().getLevelList();
	
	int totalCount = 0;
	totalCount = levelArray.size();
%>
<html>
<head>
<title>
...
<body>
	<div id="conTents">
		<form name="LForm" method="post">
			<%=function.getTitleBox(ADMIN_IMG, "권한관리 (" + totalCount + ")")%>
			<table width="100%" cellSpacing="0" cellPadding="0" border="0" class="t_b_bottom">
				<tr>
					...
					<th width="" class="t_th">권한 이름</th>
					<th width="20%" class="t_th">권한 ID</th>
				...
				<%
						...
						for(int i = 0; i < levelArray.size(); i++) {
							...
							int intIdx = level.getLevIdx();
							String strId = level.getLevId();
							String strName = level.getLevName();
							String strCheck = level.getLevStatus();
			...
			<%=function.getTipBoxTop(ADMIN_IMG)%>
				...
				사용을 하실 권한은 상태를 사용으로 바꿔 주세요.<br />
				<font color="red">권한 ID는 중복되지 않도록 해주세요!</font>
