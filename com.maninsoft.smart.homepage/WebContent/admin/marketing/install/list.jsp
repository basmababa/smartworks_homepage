...
<%
	...
	String strOrder = " order by ins_idx desc";
	
	//전체 게시물의 개수를 받아온다.
	int totalCount = 0;
	totalCount = insmanager.getInstallListCount(strWhere);
	
	//전체 페이지의수
	int pageCount = (totalCount - 1) / pageSize + 1;
	
	//해당 페이지의 리스트를 받아온다.
	ArrayList insArray = insmanager.getInstallList(req, pageSize, strWhere, strOrder);
...
<body>
	<div id="conTents">
		<form name="LForm" method="post" action="list.jsp">
			...
			<%=function.getTitleBox(ADMIN_IMG, "설치신청 관리(" + totalCount + ")")%>
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="t_b_bottom">
								...
								<tr>
									...
									<td>
										<select name="searchType">
											<option value="ins_company" <%if (searchType.equals("ins_company")) { out.println("selected"); }%>>회사명</option>
											<option value="ins_name" <%if (searchType.equals("ins_name")) { out.println("selected"); }%>>담당자</option>
											<option value="ins_email" <%if (searchType.equals("ins_email")) { out.println("selected"); }%>>이메일</option>
											<option value="ins_hp" <%if (searchType.equals("ins_hp")) { out.println("selected"); }%>>연락처</option>
										</select>
				...
				<tr height="25">
					...
					<th width="10%" class="t_th">직원수<b>/</b>사용자수</th>
					<th class="t_th">업종<b>/</b>업태</th>
					<th width="10%" class="t_th">서버위치</th>
					<th width="10%" class="t_th">네트워크환경</th>
	        	...
	        	<%
						...
						for (int i = 0; i < insArray.size(); i++) {
							...
							install = (InstallBean) insArray.get(i);
		
							int intInsIdx = install.getInsIdx();
							String strInsCompany = convert.convertStringText(install.getInsCompany() != null ? install.getInsCompany() : "");
							String strInsName = convert.convertStringText(install.getInsName() != null ? install.getInsName() : "");
							String strInsEmail = convert.convertStringText(install.getInsEmail() != null ? install.getInsEmail() : "");
							String strInsHp = convert.convertStringText(install.getInsHp() != null ? install.getInsHp() : "");
							int intInsStaffNumber = install.getInsStaffNumber();
							int intInsUserNumber = install.getInsUserNumber();
							String strInsItem = convert.convertStringText(install.getInsItem() != null ? install.getInsItem() : "");
							String strInsBusiness = convert.convertStringText(install.getInsBusiness() != null ? install.getInsBusiness() : "");
							String strInsServerLocation = install.getInsServerLocation() != null ? install.getInsServerLocation() : "";
							String strInsNetwork = install.getInsNetwork() != null ? install.getInsNetwork() : "";
							String strInsWdate = install.getInsWdate().toString().substring(0, 16);
							
							String msgServerLocation = "";
							if (strInsServerLocation.equals("1")) {
								msgServerLocation = "회사 내";
							} else if (strInsServerLocation.equals("2")) {
								msgServerLocation = "IDC";
							} else {
								msgServerLocation = "기타";
							}
	        	...
	        	%>       
					<tr>
						<td class="t_tdFirst"><input type="checkbox" name="chkSel" value="<%=intInsIdx %>"></td>
						<td class="t_td"><%=strInsCompany %></td>
						<td class="t_td"><%=strInsName %></td>
						<td class="t_td">
							<a href="javascript:top.send_mail('<%=strInsEmail %>');"><%=strInsEmail %></a>
						</td>
						<td class="t_td"><%=strInsHp %></td>
						<td class="t_td"><%=intInsStaffNumber %><b>/</b><%=intInsUserNumber %></td>
						<td class="t_td"><%=strInsItem %><b>/</b><%=strInsBusiness %></td>
						<td class="t_td"><%=msgServerLocation %></td>
						<td class="t_td"><%=msgNetwork %></td>
						<td class="t_td"><%=strInsWdate %></td>
					</tr>
				<%
						} //for (int i = 0; i < insArray.size(); i++) {
					} //if (insArray.size() == 0) {
